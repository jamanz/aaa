...
DriveApi = autoclass("com.google.android.gms.drive.DriveApi")
Sheets = autoclass("com.google.android.gms.common.Scopes.DRIVE_APPFOLDER")
    ...
    def google_activity_listener(self, request_code, result_code, data):
        if request_code == RC_SIGN_IN:
            ...
            try:
                ...
                Logger.info(f"KivyAuth: got account {account}.")
                credentials = account.getResult(ApiException)
                Logger.info(f"KivyAuth: got creds {credentials}.")
                credentials = credentials.requestScopes([Sheets, DriveApi.SCOPE_APPFOLDER])
                Logger.info(f"KivyAuth: got creds upd {credentials}.")
...
def initialize_google(success_listener, error_listener, *args, **kwargs):
    ...
    print('SS requesting')
    gso = GsoBuilder(Gso.DEFAULT_SIGN_IN)\
        .requestEmail() \
        .requestScopes(autoclass("com.google.android.gms.auth.api.scope.DriveFile")) \
        .requestScopes(autoclass("com.google.android.gms.auth.api.scope.Drive")) \
        .requestScopes(autoclass("com.google.android.gms.auth.api.scope.DriveAppFolder")) \
        .requestScopes(autoclass("com.google.android.gms.auth.api.scope.SheetsScopes.SPREADSHEETS")) \
        .build()
    print('SSend requesting')
